--[[
    Classe Personagem (Herói)
    Representa o personagem jogável principal
    Gerencia movimento, animações, tiros e colisões
]]--
...
function Personagem:new(x, y)
    ...
    -- Carrega e configura o sprite sheet
    ...
    self.largura = self.largura_animacao/8          -- 8 frames horizontais
    self.altura = self.altura_animacao/5            -- 5 linhas de animação

    -- Cria grid de animações e define diferentes estados
    ...
    -- Estados e controles do personagem
    self.estado = 'parado_dir'                      -- Estado inicial: parado olhando para direita
    self.atirando = false                           -- Flag de animação de tiro
    self.posicao = Vector(x, y) + Vector(self.largura/2, self.altura/2)  -- Posição central
    self.pos_real = Vector(x, y)                    -- Posição real (canto superior esquerdo)
    self.tiros = {}                                 -- Array de projéteis ativos
    self.delay_tiro = 0                             -- Cooldown entre tiros

    -- Atributos de combate e movimento
    self.raio = 20                                  -- Raio de colisão
    self.raio_tiro = 450                            -- Alcance de detecção
    self.vida = 100                                 -- Pontos de vida
    self.vel = 230                                  -- Velocidade de movimento
    self.dano = 30                                  -- Dano causado por tiro

    -- Controles de colisão
    self.estado_anterior = nil                      -- Guarda estado antes de colidir
    self.tempo_colisao = 0                          -- Timer de knockback
    self.vetor_direcao = Vector(0, 0)              -- Direção do knockback

    -- Collider físico (usando Windfield)
    self.collider = world:newBSGRectangleCollider(self.pos_real.x+30, self.pos_real.y+10, self.largura-60, self.altura-80, 10)
    self.collider:setFixedRotation(true)           -- Impede rotação do collider
end

--[[
    Atualiza o estado do personagem a cada frame
    Processa input, movimento, colisões e tiros
    @param dt: Delta time
]]--
function Personagem:update(dt)
    -- Define estado padrão baseado na última direção
    ...
    local vx = 0                                    -- Velocidade horizontal
    local vy = 0                                    -- Velocidade vertical

    -- PROCESSAMENTO DE INPUT - Movimento Horizontal
    ...
    -- SISTEMA DE KNOCKBACK - Aplica recuo quando colide com boss
    ...
    for i=1, #inimigos do
        if verifica_colisao(heroi.posicao, heroi.raio, inimigos[i].posicao, inimigos[i].raio) then
            ...
            -- Aplica dano se não estiver em cooldown
    ...
    -- SISTEMA DE DISPARO - Só atira se estiver parado
    ...
    if love.mouse.isDown(1) and not self.atirando and tmp_cond_parado and mouse_delay <=0 then
        ...
        local tiro
        -- Cria projétil na direção que o personagem está olhando
    ...
    -- SISTEMA DE COLISÃO DE PROJÉTEIS
    -- Cria lista unificada de todos os alvos possíveis
    local itens_mapa = {}
    ...
    -- Ordena por posição X para otimizar detecção de colisão
